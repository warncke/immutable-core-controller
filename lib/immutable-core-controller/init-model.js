...
function initModel (args) {
    ...
    basePath.get = {
        ...
        // default method returns input with data loaded
...
function schema (controller, args) {
    ...
    if (args.action) {
        ...
        if (!model.actionModels[args.action]) {
            httpError(400, 'Invalid action '+args.action)
        }
    ...
    // get schema
    var schemaMeta = model.schemaMeta
    // if meta is set or there is no data column return entire schema 
    // otherwise return only schema for data
    return args.meta || !dataColumn
        ? schemaMeta
        : schemaMeta.properties[dataColumn].properties
}
