...
function buildFormLink (args, controller, form, instance) {
    ...
    if (form.inputType === 'select') {
        ...
        var optionValueProperty = input.optionValueProperty || 'id'
        // option title is either specified or it - should be specified
        var optionTitleProperty = input.optionTitleProperty || 'id'
        ...
        .then(records => {
            ...
            var options = _.map(records, record => {
                return {
                    value: record[optionValueProperty],
                    title: record[optionTitleProperty] || record.data && record.data[optionTitleProperty]
                }
            })
            // build field
            var field = {
